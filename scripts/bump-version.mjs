...
try {
  ...
  // Matches any pinned tag, not `v${currentVersion}`: the README may have
  // missed earlier releases, and then the tag it carries is not the one the
  // package.json is being bumped from.
  const pinnedImageTag = /(ghcr\.io\/gontrum\/signalform:v)\d+\.\d+\.\d+/g;
  const newVersionTag = `v${newVersion}`;

  if (pinnedImageTag.test(readme)) {
    readme = readme.replace(pinnedImageTag, `$1${newVersion}`);
    writeFileSync(readmePath, readme);
    console.log(`✓ Updated README.md (→ ${newVersionTag})`);
  } else {
