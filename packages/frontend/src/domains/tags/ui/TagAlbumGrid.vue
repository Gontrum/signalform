...
const emit = defineEmits<{
  ...
  (event: 'album-click', album: TagAlbum, key: string): void
}>()
...
const errorMessage = computed(() =>
  props.errorKind === null ? '' : t(ERROR_KEYS[props.errorKind]),
)
      ...
      <ul data-testid="tag-albums-grid" class="grid grid-cols-2 gap-6 lg:grid-cols-3 lg:gap-8">
        ...
        >
          <button
            ...
            @click="emit('album-click', album, albumKey(album, index))"
          >
            <div
            ...
            <p data-testid="tag-album-title" class="truncate text-sm font-medium text-neutral-900">
              {{ album.title }}
            </p>
